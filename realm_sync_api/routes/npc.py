...
class NPCRetriever(RealmSyncRetriever[NPC, ListRequestArgs]):
    ...
    async def list(self, body: ListRequestArgs = ListRequestArgs()) -> list[NPC]:
        redis_client = get_redis_client()
        npcs = []
        # Scan for all keys matching the npc pattern
        cursor = 0
        while True:
            cursor, keys = await redis_client.scan(cursor, match="npc:*", count=100)
            for key in keys:
                data = await redis_client.get(key)
                if data:
    ...
    async def create(self, data: NPC) -> NPC:
        redis_client = get_redis_client()
    ...
    async def update(self, id: str, data: NPC) -> NPC:
        redis_client = get_redis_client()
        key = self._get_key(id)
