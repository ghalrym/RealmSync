...
def test_player_retriever_get_success(mock_redis):
    ...
    mock_redis.get.return_value = player_data.model_dump_json()
...
def test_player_retriever_get_not_found(mock_redis):
...
def test_player_retriever_list_empty(mock_redis):
    """Test listing players when there are none."""
    set_redis_client(mock_redis)
    mock_redis.scan.return_value = (0, [])

    retriever = PlayerRetriever()
    result = retriever.list(type(retriever).__orig_bases__[0].__args__[1]())

    assert result == []


def test_player_retriever_list_with_players(mock_redis):
...
def test_player_retriever_create_success(mock_redis):
    ...
    retriever = PlayerRetriever()
    result = retriever.create(player)
...
def test_player_retriever_create_duplicate(mock_redis):
    ...
    with pytest.raises(ValueError, match="Player with id '1' already exists"):
        retriever.create(player)


def test_player_retriever_create_calls_hook(mock_redis):
...
def test_player_retriever_update_success(mock_redis):
    ...
    retriever = PlayerRetriever()
    result = retriever.update("1", player)
...
def test_player_retriever_update_not_found(mock_redis):
    ...
    with pytest.raises(ValueError, match="Player with id '1' not found"):
        retriever.update("1", player)


def test_player_retriever_update_id_mismatch(mock_redis):
...
def test_player_retriever_update_calls_hook(mock_redis):
...
def test_player_retriever_delete_success(mock_redis):
...
def test_player_retriever_delete_not_found(mock_redis):
...
def test_player_retriever_delete_calls_hook(mock_redis):
