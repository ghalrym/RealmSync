...
from realm_sync_api.routes.item import ItemRetriever, router
...
def test_item_retriever_get_success(mock_redis):
    ...
    mock_redis.get.return_value = item_data.model_dump_json()
...
def test_item_retriever_get_not_found(mock_redis):
...
def test_item_retriever_list_empty(mock_redis):
    """Test listing items when there are none."""
    set_redis_client(mock_redis)
    mock_redis.scan.return_value = (0, [])

    retriever = ItemRetriever()
    from realm_sync_api.routes.item import ListRequestArgs
...
def test_item_retriever_create_success(mock_redis):
...
def test_item_retriever_create_duplicate(mock_redis):
    ...
    with pytest.raises(ValueError, match="Item with id '1' already exists"):
        retriever.create(item)


def test_item_retriever_update_success(mock_redis):
    ...
    retriever = ItemRetriever()
    result = retriever.update("1", item)
...
def test_item_retriever_update_not_found(mock_redis):
    ...
    with pytest.raises(ValueError, match="Item with id '1' not found"):
        retriever.update("1", item)


def test_item_retriever_update_id_mismatch(mock_redis):
...
def test_item_retriever_delete_success(mock_redis):
...
def test_item_retriever_delete_not_found(mock_redis):
